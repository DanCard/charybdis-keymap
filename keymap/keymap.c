...
const char* get_rgb_mode_name(uint8_t mode) {
    switch (mode) {
        case RGB_MATRIX_SOLID_COLOR: return "SOLID_COLOR";
        case RGB_MATRIX_ALPHAS_MODS: return "ALPHA_MODS";
        case RGB_MATRIX_GRADIENT_UP_DOWN: return "GRADIENT_UP_DOWN";
        case RGB_MATRIX_GRADIENT_LEFT_RIGHT: return "GRADIENT_LEFT_RIGHT";
        case RGB_MATRIX_BREATHING: return "BREATHING";
        case RGB_MATRIX_BAND_SAT: return "COLORBAND_SAT";
        case RGB_MATRIX_BAND_VAL: return "COLORBAND_VAL";
        case RGB_MATRIX_BAND_PINWHEEL_SAT: return "COLORBAND_PINWHEEL_SAT";
        case RGB_MATRIX_BAND_PINWHEEL_VAL: return "COLORBAND_PINWHEEL_VAL";
        case RGB_MATRIX_BAND_SPIRAL_SAT: return "COLORBAND_SPIRAL_SAT";
        case RGB_MATRIX_BAND_SPIRAL_VAL: return "COLORBAND_SPIRAL_VAL";
        case RGB_MATRIX_CYCLE_ALL: return "CYCLE_ALL";
        case RGB_MATRIX_CYCLE_LEFT_RIGHT: return "CYCLE_LEFT_RIGHT";
        case RGB_MATRIX_CYCLE_UP_DOWN: return "CYCLE_UP_DOWN";
        case RGB_MATRIX_CYCLE_OUT_IN: return "CYCLE_OUT_IN";
        case RGB_MATRIX_CYCLE_OUT_IN_DUAL: return "CYCLE_OUT_IN_DUAL";
        case RGB_MATRIX_RAINBOW_MOVING_CHEVRON: return "RAINBOW_MOVING_CHEVRON";
        case RGB_MATRIX_CYCLE_PINWHEEL: return "CYCLE_PINWHEEL";
        case RGB_MATRIX_CYCLE_SPIRAL: return "CYCLE_SPIRAL";
        case RGB_MATRIX_DUAL_BEACON: return "DUAL_BEACON";
        case RGB_MATRIX_RAINBOW_BEACON: return "RAINBOW_BEACON";
        case RGB_MATRIX_RAINBOW_PINWHEELS: return "RAINBOW_PINWHEELS";
        case RGB_MATRIX_RAINDROPS: return "RAINDROPS";
        case RGB_MATRIX_JELLYBEAN_RAINDROPS: return "JELLYBEAN_RAINDROPS";
        case RGB_MATRIX_HUE_BREATHING: return "HUE_BREATHING";
        case RGB_MATRIX_HUE_PENDULUM: return "HUE_PENDULUM";
        case RGB_MATRIX_HUE_WAVE: return "HUE_WAVE";
        case RGB_MATRIX_PIXEL_FRACTAL: return "PIXEL_FRACTAL";
        case RGB_MATRIX_PIXEL_FLOW: return "PIXEL_FLOW";
        case RGB_MATRIX_PIXEL_RAIN: return "PIXEL_RAIN";
        case RGB_MATRIX_TYPING_HEATMAP: return "TYPING_HEATMAP";
        case RGB_MATRIX_DIGITAL_RAIN: return "DIGITAL_RAIN";
        case RGB_MATRIX_SOLID_REACTIVE_SIMPLE: return "SOLID_REACTIVE_SIMPLE";
        case RGB_MATRIX_SOLID_REACTIVE: return "SOLID_REACTIVE";
        case RGB_MATRIX_SOLID_REACTIVE_WIDE: return "SOLID_REACTIVE_WIDE";
        case RGB_MATRIX_SOLID_REACTIVE_CROSS: return "SOLID_REACTIVE_CROSS";
        case RGB_MATRIX_SOLID_REACTIVE_MULTICROSS: return "SOLID_REACTIVE_MULTICROSS";
        case RGB_MATRIX_SOLID_REACTIVE_NEXUS: return "SOLID_REACTIVE_NEXUS";
        case RGB_MATRIX_SOLID_REACTIVE_MULTINEXUS: return "SOLID_REACTIVE_MULTINEXUS";
        case RGB_MATRIX_SPLASH: return "SPLASH";
        case RGB_MATRIX_MULTISPLASH: return "MULTISPLASH";
        case RGB_MATRIX_SOLID_SPLASH: return "SOLID_SPLASH";
        case RGB_MATRIX_SOLID_MULTISPLASH: return "SOLID_MULTISPLASH";
    ...
    switch (keycode) {
        ...
        case KC_RGB_AUTO:
            if (record->event.pressed) {
                ...
                // Not syncing this explicitly as it's a background process, but maybe we should?
                // Let's rely on master driving animations if synced.
            }
            return false;
        case RM_HUEU:
        case RM_HUED:
        ...
        case KC_MINS_TO0:
            if (record->event.pressed) {
                ...
                mins_held = false;
                if (!mins_triggered) { tap_code(KC_MINS); }
            }
            return false;
        case KC_0_TG1:
            if (record->event.pressed) {
                k0_held = true; k0_triggered = false; k0_tap_timer = timer_read();
            } else {
                k0_held = false;
                if (!k0_triggered) { tap_code(KC_0); }
            }
            return false;
        case KC_9_TG2:
            if (record->event.pressed) {
                k9_held = true; k9_triggered = false; k9_tap_timer = timer_read();
            } else {
                k9_held = false;
                if (!k9_triggered) { tap_code(KC_9); }
            }
            return false;
        case KC_8_TG3:
            if (record->event.pressed) {
                k8_held = true; k8_triggered = false; k8_tap_timer = timer_read();
            } else {
                k8_held = false;
                if (!k8_triggered) { tap_code(KC_8); }
            }
            return false;
        case KC_7_TO0:
            if (record->event.pressed) {
                k7_held = true; k7_triggered = false; k7_tap_timer = timer_read();
            } else {
                k7_held = false;
                if (!k7_triggered) { tap_code(KC_7); }
            }
            return false;
        case KC_6_TO0:
            if (record->event.pressed) {
                k6_held = true; k6_triggered = false; k6_tap_timer = timer_read();
            } else {
                k6_held = false;
...
void matrix_scan_user(void) {
    ...
    if (mins_held && !mins_triggered && timer_elapsed(mins_tap_timer) > MY_TAPPING_TERM) {
        layer_move(0);
        mins_triggered = true;
        rgb_matrix_indicators_user();
    }
    if (k0_held && !k0_triggered && timer_elapsed(k0_tap_timer) > MY_TAPPING_TERM) {
        if (get_highest_layer(layer_state) == 0) { layer_move(1); } else { layer_move(0); } // Toggle logic like Base
        k0_triggered = true;
        rgb_matrix_indicators_user();
    }
    if (k9_held && !k9_triggered && timer_elapsed(k9_tap_timer) > MY_TAPPING_TERM) {
        if (get_highest_layer(layer_state) == 0) { layer_move(2); } else { layer_move(0); }
        k9_triggered = true;
        rgb_matrix_indicators_user();
    }
    if (k8_held && !k8_triggered && timer_elapsed(k8_tap_timer) > MY_TAPPING_TERM) {
        if (get_highest_layer(layer_state) == 0) { layer_move(3); } else { layer_move(0); }
        k8_triggered = true;
        rgb_matrix_indicators_user();
    }
    if (k7_held && !k7_triggered && timer_elapsed(k7_tap_timer) > MY_TAPPING_TERM) {
        layer_move(0);
        k7_triggered = true;
        rgb_matrix_indicators_user();
    }
    if (k6_held && !k6_triggered && timer_elapsed(k6_tap_timer) > MY_TAPPING_TERM) {
        layer_move(0);
        k6_triggered = true;
        rgb_matrix_indicators_user();
    }
