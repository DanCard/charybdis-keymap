...
static const rgb_mode_key_t rgb_mode_keys[] = {
    {KC_RAINBOW, RGB_MATRIX_CYCLE_LEFT_RIGHT}, {KC_REACTIVE, RGB_MATRIX_SPLASH}, {KC_JELLY, RGB_MATRIX_JELLYBEAN_RAINDROPS}, {KC_SPIRAL, RGB_MATRIX_CYCLE_SPIRAL}, {KC_CHEVRON, RGB_MATRIX_RAINBOW_MOVING_CHEVRON},
};
...
tap_dance_action_t tap_dance_actions[] = {[TD_Z_LAYER] = ACTION_TAP_DANCE_FN_ADVANCED(NULL, z_finished, z_reset)};
...
layer_state_t layer_state_set_user(layer_state_t state) {
  ...
  uprintf("[%lu.%03lu] Layer change: %u -> %u (state=%lu) Reason: %s\n", sec, ms, prev_layer, layer, (unsigned long)state, layer_change_reason ? layer_change_reason : "Standard Keycode / Core");
...
static bool handle_l1_arrow_overrides(uint16_t keycode, keyrecord_t *record) {
  ...
  if (is_right_side) {
    ...
    if (!shift_held && !ctrl_held) return true;
  ...
  switch (keycode) {
    ...
    case KC_ENT_EXIT:
      if (record->event.pressed) {
        ...
        uprintf("KC_ENT_EXIT Released. Elapsed: %u. Action: %s\n", timer_elapsed(th[TH_ENT_MO].timer), (timer_elapsed(th[TH_ENT_MO].timer) < TAPPING_TERM) ? "Tap (Exit)" : "Hold (Exit)");
      ...
      return false;
    case KC_SPC_EXIT:
      return handle_exit_key(KC_SPC, record->event.pressed);
    case KC_BSPC_EXIT:
      return handle_exit_key(KC_BSPC, record->event.pressed);
    case KC_L1_L3:
      if (record->event.pressed) {
        th[TH_L1].held      = true;
    ...
    case KC_ENT_L2:
      return handle_thumb_toggle(TH_ENT_TG2, KC_ENT, "KC_ENT_L2", record->event.pressed);
    case KC_ENT_L4:
      return handle_thumb_toggle(TH_ENT_TG4, KC_ENT, "KC_ENT_L4", record->event.pressed);
    case KC_SPC_L2:
      return handle_thumb_toggle(TH_SPC_TG2, KC_SPC, "KC_SPC_L2", record->event.pressed);
    case KC_SPC_L4:
      return handle_thumb_toggle(TH_SPC_TG4, KC_SPC, "KC_SPC_L4", record->event.pressed);
    ...
    case KC_DAY:
      if (record->event.pressed) {
        ...
        // Keep current Hue/Sat, set Value to DAY_BRIGHTNESS
        rgb_matrix_sethsv(rgb_matrix_get_hue(), rgb_matrix_get_sat(), DAY_BRIGHTNESS);
        is_day_mode = true;
        sync_needed = true;
        uprintf("[%lu.%03lu] Day Mode: Brightness set to %d (Saved to EEPROM)\n", sec, ms, DAY_BRIGHTNESS);
        layer_history_pop();
        rgb_matrix_indicators_user();
      }
      return false;
    case KC_NIGHT:
      if (record->event.pressed) {
        ...
        // Keep current Hue/Sat, set Value to NIGHT_BRIGHTNESS
        rgb_matrix_sethsv(rgb_matrix_get_hue(), rgb_matrix_get_sat(), NIGHT_BRIGHTNESS);
        is_day_mode = false;
        sync_needed = true;
        uprintf("[%lu.%03lu] Night Mode: Brightness set to %d (Saved to EEPROM)\n", sec, ms, NIGHT_BRIGHTNESS);
        layer_history_pop();
        rgb_matrix_indicators_user();
      }
      return false;
    ...
    case KC_L3_EXT_TO4:
      return handle_l3_thumb(TH_L3_TO4, 0, 4, record->event.pressed);
    case KC_L3_EXT_TO2:
      return handle_l3_thumb(TH_L3_TO2, 0, 2, record->event.pressed);
    case KC_L3_EXT_TO1:
      return handle_l3_thumb(TH_L3_TO1, 1, 6, record->event.pressed);
    case KC_EXIT_TO3:
      return handle_l3_thumb(TH_EXIT_TO3, 0, 3, record->event.pressed);
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [0] = LAYOUT(QK_GESC, KC_1_L1, KC_2_L2, KC_3_L3, KC_4_L4, KC_5_L5, KC_6, KC_7, KC_8, KC_9, KC_0, KC_MINS_EQL, KC_TAB, KC_Q_L4, KC_W, KC_E, KC_R, KC_T, KC_Y, KC_U, KC_I, KC_O, KC_P, KC_BSLS, KC_LSFT, KC_A, KC_S, KC_D, KC_F, KC_G, KC_H, KC_J, KC_K, KC_L, KC_PLUS_COLON, KC_QUOT, KC_LCTL, TD(TD_Z_LAYER), KC_X, KC_C, KC_V, KC_B, KC_N, KC_M, KC_COMM, KC_DOT, KC_SLSH, KC_RSFT, KC_SPC_L4, KC_ENT_L2, KC_L1_L3, KC_DEL, KC_ENT_L2, KC_LALT, KC_BSPC, KC_BSPC),
    // Layer 1: Right Arrows
    [1] = LAYOUT(QK_GESC, KC_1_L1, KC_2_L2, KC_3_L3, KC_4_L4, KC_5_L5, KC_6, KC_7, KC_8, KC_9, KC_0_TO0, KC_MINS_EQL, KC_TAB, KC_Q_Z, KC_W, KC_E, KC_R, KC_T, KC_Y, KC_U, KC_I, KC_O, KC_P, KC_BSLS, KC_LSFT, KC_A, KC_S, KC_D, KC_F, KC_G, KC_H, KC_J, KC_K, KC_L, KC_PLUS_COLON, KC_QUOT, KC_LCTL, KC_LEFT, KC_X, KC_C, KC_V, KC_B, KC_N, KC_M, KC_COMM, KC_DOT, KC_SLSH, KC_RSFT, KC_SPC_L4, KC_ENT_L2, KC_UP, KC_DEL, KC_ENT_L2, KC_DOWN, KC_RIGHT, KC_BSPC),
    [2] = LAYOUT(KC_EXIT, KC_EXIT, KC_EXIT, KC_EXIT, KC_EXIT, KC_EXIT, KC_EXIT, KC_EXIT, KC_EXIT, KC_EXIT, KC_EXIT, QK_BOOT, KC_PSCR, KC_EXIT, KC_EXIT, KC_EXIT, HYPR(KC_N), QK_BOOT, KC_EXIT, KC_LBRC, KC_RBRC, S(KC_LBRC), S(KC_RBRC), KC_EXIT, KC_LSFT, KC_LEFT, KC_UP, KC_DOWN, KC_RGHT, LALT(KC_HOME), KC_EXIT, KC_LEFT, KC_UP, KC_DOWN, KC_RGHT, KC_EXIT, KC_LCTL, KC_HOME, KC_PGUP, KC_PGDN, KC_END, KC_EXIT, KC_EXIT, KC_HOME, KC_PGUP, KC_PGDN, KC_END, KC_RSFT, KC_SPC_EXIT, KC_ENT_EXIT, KC_L1_L3, KC_EXIT, KC_ENT_EXIT, KC_LALT, KC_BSPC_EXIT, KC_BSPC_EXIT),
    [3] = LAYOUT(QK_GESC, KC_EXIT, KC_EXIT, KC_EXIT, KC_EXIT, QK_BOOT, KC_EXIT, KC_RCTL, KC_RALT, KC_RGUI, KC_EXIT, QK_BOOT, KC_SEL_LOCK, CM_MS_UL, CM_MS_UP, CM_MS_UR, MS_BTN2, KC_EXIT, KC_EXIT, KC_SNIPE, KC_FAST, KC_EXIT, KC_EXIT, QK_CLEAR_EEPROM, KC_EXIT, CM_MS_LEFT, SCROLL_MODE, CM_MS_RIGHT, MS_BTN1, MS_BTN2, KC_MOUSE_LOCK, MS_BTN1, SCROLL_MODE, KC_SEL_LOCK, MS_BTN2, KC_EXIT, KC_EXIT, CM_MS_DL, CM_MS_DOWN, CM_MS_DR, MS_BTN3, KC_EXIT, KC_EXIT, KC_EXIT, MS_BTN3, MS_BTN3, MS_BTN3, KC_RSFT, MS_BTN1, KC_L3_EXT_TO2, KC_L3_EXT_TO1, KC_MOUSE_LOCK, KC_ENT_EXIT, KC_LALT, KC_BSPC_EXIT, KC_BSPC_EXIT),
    // Layer 4: left hand layer
    [4] = LAYOUT(KC_MINS_TO0, KC_0_L1, KC_9_L2, KC_8_L3, KC_7_TO0, KC_6_TO0, KC_6, KC_7, KC_8, KC_9, KC_0_TO0, KC_MINS_EQL, KC_BSLS, KC_P_TO0, KC_O, KC_I, KC_U, KC_Y, KC_Y, KC_U, KC_I, KC_O, KC_P, KC_BSLS, KC_QUOT, KC_PLUS_COLON, KC_L, KC_K, KC_J, KC_H, KC_H, KC_J, KC_K, KC_L, KC_PLUS_COLON, KC_QUOT, KC_LCTL, KC_SLSH, KC_DOT, KC_COMM, KC_M, KC_N, KC_N, KC_M, KC_COMM, KC_DOT, KC_SLSH, KC_RCTL, KC_SPC_EXIT, KC_ENT_EXIT, KC_LSFT, KC_R_L2, KC_ENT_EXIT, KC_LALT, KC_BSPC, KC_BSPC),
    // Layer 5: Settings Layer - accessed via long press 6
    [5] = LAYOUT(KC_F1, KC_F2, KC_F3, KC_F4, KC_F5, KC_F6, KC_F7, KC_F8, KC_F9, KC_F10, KC_F11, KC_F12, KC_EXIT, RM_TOGG, RM_NEXT, RM_PREV, KC_RGB_AUTO, KC_EXIT, KC_DUMP_LOG, KC_PRINT_STATS, KC_LOG_STATS_GRID, KC_DBG_TOG, KC_EXIT, KC_EXIT, KC_EXIT, KC_FLASHLIGHT, RM_VALU, RM_VALD, KC_DAY, KC_NIGHT, KC_EXIT, DPI_MOD, DPI_RMOD, KC_JITTER, KC_EXIT, KC_EXIT, KC_PSCR, RM_HUEU, RM_HUED, RM_SATU, RM_SATD, KC_FIRE, KC_EXIT, KC_MS_TMO_INC, KC_MS_TMO_DEC, KC_EXIT, KC_EXIT, KC_PSCR, KC_EXIT, KC_EXIT, KC_EXIT, KC_EXIT, KC_EXIT, KC_EXIT, KC_EXIT, KC_EXIT),
...
void keyboard_post_init_user(void) {
  ...
  uprintf("Init: EEPROM Read=%u (Theme %d, Hue %d, Jitter %d, Log %d)\n", user_config, saved_theme, saved_hue, is_jitter_filter_active, is_sync_logging_enabled);
  ...
  uprintf("Init: Next Theme=%d, Next Hue=%d. Saved=%u. CPI: %u\n", next_theme, next_hue, new_config, pointing_device_get_cpi());
